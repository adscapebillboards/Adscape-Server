...
async function manualSlotGeneration() {
  try {
    ...
    for (const billboard of billboards) {
      if (billboard.status?.toUpperCase() === 'APPROVED') {
        ...
        if (existingSlots === 0) {
          ...
          // Generate slots
          try {
            const start = new Date(billboard.bookingDetails.startDate);
    ...
    console.error('❌ Manual slot generation failed:', error);
  } finally {
    await prisma.$disconnect();
  }
}

// Run the manual slot generation
if (require.main === module) {
  manualSlotGeneration();
}

module.exports = { manualSlotGeneration };
